...
* Load detection content only if it is signed by a key you trust (unless you
  explicitly allow unsigned content).
...
* No archive scanning: malware inside ZIP or other containers is not
  detected.
* No quarantine on Windows. Quarantine does not stop running processes or
  remove persistence.
* No update mechanism, and no rollback protection for signed content.
